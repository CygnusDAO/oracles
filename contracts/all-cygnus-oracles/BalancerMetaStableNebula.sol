...
/*  ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
     ...
     🛸   ███░░░░░███                                              📡                                     🌔   
        ...
        CYGNUS LP ORACLE (Balancer MetaStable Pools) - https://cygnusdao.finance                                                          .                     .
    ═══════════════════════════════════════════════════════════════════════════════════════════════════════════ */
 ...
 *  @notice Implementation of fair BPT pricing for metastable stable pools
 */
contract BalancerMetaStableNebula is ICygnusNebula {
    /*  ═══════════════════════════════════════════════════════════════════════════════════════════════════════ 
            1. LIBRARIES
        ═══════════════════════════════════════════════════════════════════════════════════════════════════════  */
     ...
     *  @inheritdoc ICygnusNebula
     */
    string public override name = "Cygnus: Balancer MetaStable Nebula";
    ...
    function lpTokenPriceUsd(address lpTokenPair) external view override context(VAULT) returns (uint256 lpTokenPrice) {
        ...
        for (uint256 i = 0; i < oracle.poolTokens.length; i++) {
            ...
            (uint256 tokenRate, , ) = IMetaStablePool(lpTokenPair).getPriceRateCache(address(oracle.poolTokens[i]));

            // The base asset (WETH, USDC) always has a tokenRate of 0, so we leave unadjusted to 18 decimals
            uint256 adjustedPrice = tokenRate != 0 ? assetPrice.div(tokenRate) : assetPrice;
        ...
        uint256 rate = IMetaStablePool(lpTokenPair).getRate();
        ...
        )
    {
        ...
        bytes32 poolId = IMetaStablePool(lpTokenPair).getPoolId();
